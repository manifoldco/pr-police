...
module.exports = function server() {
  ...
  requiredEnvs.forEach(token => {
    if (!env[token] || !env[token].length) throw new Error(`Missing required ENV var ${token}`);
  });
      ...
      if (daysToRun.toLowerCase().indexOf(now.format('dddd').toLowerCase()) !== -1) {
        for (var i = timesToRun.length; i--; ) {
          if (parseInt(timesToRun[i]) === parseInt(now.format('kmm'))) {
            ...
            if (i === 0) {
              DEBUG &&
                console.log(now.format('kmm'), `does not match any TIMES_TO_RUN (${timesToRun})`);
            }
          }
        }
      } else {
  ...
  function hasLabel(PR, label) {
    return PR.labels.some(({ name }) => name === label);
  }

  function buildMessage(data) {
    if (!data) return Promise.resolve(messages.GITHUB_ERROR[bot.name]);
    ...
    // sort by date
    whitelisted.sort((a, b) => a.created_at - b.created_at);

    whitelisted.forEach(PR => {
      if (hasLabel(PR, blockedLabel)) {
        blocked.push(PR);
      } else if (hasLabel(PR, wipLabel)) {
        inProgress.push(PR);
      } else {
        needReview.push(PR);
      }
    ...
    if (blocked.length || needReview.length || inProgress.length) {
      const format = PR => {
        const ago = moment(PR.created_at).fromNow();
        const segments = PR.repository_url.split('/');
        const repo = segments[segments.length - 1];
        return `‣ *${ago}* [${PR.user.login}] <${PR.html_url}|${repo}#${PR.number}>: ${PR.title}`;
      };

      let message = [];

      if (blocked.length) {
        message.push(messages.PR_LIST_BLOCKED[bot.name].replace(/\${NUMBER}/, blocked.length));
        message.push(...blocked.map(PR => format(PR)));
      }

      if (needReview.length) {
        message.push(messages.PR_LIST_REVIEW[bot.name].replace(/\${NUMBER}/, needReview.length));
        message.push(...needReview.map(PR => format(PR)));
      }

      if (inProgress.length) {
        message.push(messages.PR_LIST_WIP[bot.name].replace(/\${NUMBER}/, inProgress.length));
        message.push(...inProgress.map(PR => format(PR)));
      }
