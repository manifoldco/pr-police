  ...
  function buildMessage(data) {
    ...
    if (blocked.length || needReview.length || inProgress.length) {
      const format = PR => {
        ...
        return `‣ [${PR.user.login}] <${PR.html_url}|${repo}#${PR.number}>: ${PR.title}`;
      };
      ...
      if (blocked.length)
        message = [
          ...message,
          messages.PR_LIST_BLOCKED[bot.name].replace(/\${NUMBER}/, blocked.length),
      ...
      if (needReview.length)
        message = [
          ...message,
          messages.PR_LIST_REVIEW[bot.name].replace(/\${NUMBER}/, needReview.length),
      ...
      if (inProgress.length)
        message = [
          ...message,
          messages.PR_LIST_WIP[bot.name].replace(/\${NUMBER}/, inProgress.length),
