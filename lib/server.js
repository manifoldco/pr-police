...
module.exports = function server() {
  ...
  function buildMessage(data) {
    ...
    const appendPR = PR =>
      message.push(`:star: [${PR.user.login}] <${PR.html_url}|${PR.title}>`);

    data.forEach(pullRequest => {
      if (!excludeLabels.length) return appendPR(pullRequest);

      const blacklisted = pullRequest.labels.find(
        ({ name }) => excludeLabels.indexOf(name) !== -1,
      );
      if (blacklisted && DEBUG) {
        return console.log(
          `exclude label match: ${blacklisted} for: ${pullRequest.title}`,
        );
      }
      return appendPR(pullRequest);
    });

    if (message.length) {
      return Promise.resolve(
        [messages.PR_LIST_HEADER.replace(/\${NUMBER}/, message.length)]
          .concat(message)
          .join('\n'),
      );
    }
